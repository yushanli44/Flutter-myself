...
class CalendarLogic {
  ...
  final Map<CalendarFormat, String> _availableCalendarFormats;
  DateTime _previousFirstDay;
  DateTime _previousLastDay;
  int _pageId;
  double _dx;
  final bool _useNextCalendarFormat;
  ...
  CalendarLogic(
    ...
    if (onVisibleDaysChanged != null) {
      _visibleDays.addListener(() {
        if (!DateTimeUtil.isSameDay(_visibleDays.value.first, _previousFirstDay) ||
            !DateTimeUtil.isSameDay(_visibleDays.value.last, _previousLastDay)) {
          _previousFirstDay = _visibleDays.value.first;
          _previousLastDay = _visibleDays.value.last;
  ...
  bool setSelectedDay(DateTime value, {bool isAnimated = true, bool isProgrammatic = false}) {
    if (DateTimeUtil.isSameDay(value, _selectedDay)) {
      return false;
    }
  ...
  DateTime selectPrevious() {
    if (calendarFormat == CalendarFormat.month) {
      return _selectPreviousMonth();
  ...
  DateTime _selectPreviousMonth() {
    return _focusedDay = DateTimeUtil.previousMonth(_focusedDay);
  }

  DateTime _selectNextMonth() {
    return _focusedDay = DateTimeUtil.nextMonth(_focusedDay);
  }

  void _selectPreviousTwoWeeks() {
    if (_visibleDays.value.take(7).contains(_focusedDay)) {
      // in top row
      _focusedDay = DateTimeUtil.previousWeek(_focusedDay);
    } else {
      // in bottom row OR not visible
      _focusedDay = DateTimeUtil.previousWeek(_focusedDay.subtract(const Duration(days: 7)));
    }
  }
  ...
  void _selectNextTwoWeeks() {
    if (!_visibleDays.value.skip(7).contains(_focusedDay)) {
      ...
      _focusedDay = DateTimeUtil.nextWeek(_focusedDay);
    }
  }
  ...
  List<DateTime> _daysInMonth(DateTime month) {
    ...
    return DateTimeUtil.daysInRange(firstToDisplay, lastToDisplay).toList();
  }
  ...
  List<DateTime> _daysInWeek(DateTime week) {
    ...
    final days = DateTimeUtil.daysInRange(first, last);
  ...
  bool isSelected(DateTime day) {
    return DateTimeUtil.isSameDay(day, selectedDay);
  }

  bool isToday(DateTime day) {
    return DateTimeUtil.isSameDay(day, DateTime.now());
  }
  ...
  void selectPickerDate(DateTime date) {
    _focusedDay = date;
